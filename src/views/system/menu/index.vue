<template>
    <d2-container>
        <el-button-group>
            <el-button type="primary" icon="el-icon-circle-plus-outline" size="mini" @click="add">添加
            </el-button>
            <el-button type="primary" icon="el-icon-edit" :disabled="!currentId" size="mini" @click="edit">编辑
            </el-button>
            <el-button type="primary" icon="el-icon-delete" :disabled="!currentId" size="mini" @click="del">删除
            </el-button>
            <el-button type="primary" icon="el-icon-circle-close-outline" :disabled="!currentId" size="mini"
                       @click="cancel">取消
            </el-button>
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="batchEdit">
                批量编辑
            </el-button>
        </el-button-group>
        ...
        <el-row>
            <el-col :span="8" style="margin-top:15px;">
                <el-tree ref="tree" class="filter-tree" node-key="id" default-expand-all highlight-current
                         :expand-on-click-node="false" :data="menuList" :props="defaultProps"
                ...
                <el-card shadow="always">
                    <el-form label-position="right" label-width="80px" :model="menuform" ref="menuform">
                        <el-form-item label="父级节点" prop="pid">
                            <el-input v-model="menuform.pid" :disabled="true" placeholder="父级节点"/>
                        </el-form-item>
                        <el-form-item label="标题" prop="title">
                            <el-input v-model="menuform.title" :disabled="!formEdit" placeholder="请输入标题"/>
                        </el-form-item>
                        ...
                        <el-form-item label="类型" prop="type">
                            <el-select class="filter-item" v-model="menuform.type" :disabled="!formEdit"
                                       placeholder="请选择类型">
                                <el-option v-for="item in typeOptions" :key="item.velue" :label="item.label"
                                           :value="item.value"/>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="锁定" prop="isLock">
                            <el-switch v-model="menuform.isLock" :disabled="!formEdit"/>
                        </el-form-item>
    ...
    methods: {
        ...
        del () {
            this.$confirm('确认删除？', '确认信息', {
                ...
                menuService.del(this.currentId).then(() => {
                    this.currentId = 0
                    this.getMenuList()
                })
            })
